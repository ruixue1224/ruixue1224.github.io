  ...
  */
  ...
  const rightSideFn = {
    ...
    darkmode: () => { // switch between light and dark mode
      ...
      const isDayMode = willChangeMode === 'light'
      
      // 触发转场动画（仅在用户主动切换时）
      // 注意：此函数只在用户点击切换按钮时调用，不会在页面加载时触发
      if (window.dayNightTransition && typeof window.dayNightTransition.play === 'function') {
        // 播放转场动画
        window.dayNightTransition.play(isDayMode).then(() => {
          // 动画播放完成后切换主题
